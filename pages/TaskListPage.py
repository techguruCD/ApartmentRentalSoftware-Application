...
from tablemodels.TaskTableModel import TaskTableModel

tr = QtCore.QCoreApplication.translate

class TaskListPage(CustomWindow):
    ...
    def __init_UI(self):
        ...
        button_add_task = QtWidgets.QPushButton(tr('TaskListPage - Add reminder', 'Add reminder'))
        button_add_task.setObjectName('BlueButton')
        button_add_task.clicked.connect(lambda: self.Signal.emit({'window': 'reminder'}))

        self.filter_by_active = QtWidgets.QCheckBox()
        self.filter_by_active.setText(tr('TaskListPage - Show only active', 'Show only active'))
        ...
        layout.addWidget(button_add_task, 0, 1)
    ...
    def update_data(self, final_url: str = None):
        ...
        success, data = NotificationsApi.task_list(search, active, final_url)
        if success:
            self.table_model = TaskTableModel(data['results'])
            self.table_view.setModel(self.table_model)
    ...
    def __table_click(self, index: QtCore.QModelIndex):
        self.Signal.emit({'window': 'task', 'id': self.table_model._data[index.row()]['id']})
